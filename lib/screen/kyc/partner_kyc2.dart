...
class KYCScreen extends StatelessWidget {
  ...
  Widget _buildOverallStatusBar(KYCController controller) {
    return Obx(() {
  ...
  Widget _buildImportantInfoCard(KYCController controller) {
    return Container(
  ...
  Widget _buildDocumentCard(KYCController controller, DocumentRequirement doc) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Obx(() => _buildUploadButton(controller, doc)),
        ],
      ),
    );
  }
  ...
  Widget _buildSingleUploadBox(
      KYCController controller, DocumentRequirement doc) {
    return Obx(() {
      ...
      return GestureDetector(
        onTap: () => controller.pickDocument(doc.name),
        child: Container(
          height: 200,
          decoration: BoxDecoration(
            border: Border.all(
          ...
          child: (hasLocalFile || hasUploadedFile)
              ? Stack(
                  children: [
                    ClipRRect(
                      ...
                      child: hasLocalFile
                          ? Image.file(
                              controller.localFiles[doc.name]!['single']!,
                              width: double.infinity,
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: IconButton(
                          ...
                          onPressed: () => controller.pickDocument(doc.name),
                        ),
                      ),
                    ),
                  ],
              ...
              : Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      doc.acceptableFileTypes.join(', ').toUpperCase() +
                          ' (Max 5MB)',
  ...
  Widget _buildMultipleUploadItem(
    ...
    return GestureDetector(
      onTap: () => controller.pickDocument(doc.name, type: type),
      child: Container(
        height: 180,
        decoration: BoxDecoration(
          border: Border.all(
            ...
            ? Stack(
                children: [
                  ClipRRect(
                    ...
                    child: hasLocalFile
                        ? Image.file(
                            controller.localFiles[doc.name]![type]!,
                            width: double.infinity,
                            height: double.infinity,
                            fit: BoxFit.cover,
                          )
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: IconButton(
                        ...
                        onPressed: () =>
                            controller.pickDocument(doc.name, type: type),
                      ),
                    ),
                  ),
                ],
  ...
  Widget _buildUploadButton(KYCController controller, DocumentRequirement doc) {
    final canSubmit = controller.canSubmitDocument(doc.name, doc.isMultiple);

    return SizedBox(
      width: double.infinity,
      child: ElevatedButton(
        onPressed: canSubmit && !controller.isUploading.value
            ? () => controller.uploadDocument(doc.name, userId)
            : null,
  ...
  Widget _buildStatusBadge(KYCController controller, String status) {
    Color backgroundColor;
    Color textColor;
    IconData icon;
    String label;
  ...
  Widget _buildRejectionReasonBox(KYCController controller, String reason) {
    return Container(
